...
SHARED = BASE / "exports" / "QR Code Posters"   # the folder that is actually shared
...
def main() -> None:
    ...
    # Mirror into the shared folder. This used to be a manual copy and it drifted:
    # after a re-upload the poster here was current while the copy the professor
    # could actually reach still carried the dead URL. Only files this script wrote
    # are touched, so unrelated posters in that folder (e.g. the 陳外科醫院 one) are
    # left alone. Skipped for placeholder runs — never publish a dead QR.
    if placeholder:
        print(f"skipped mirroring to {SHARED.name}/ (placeholder URLs)")
        return
    SHARED.mkdir(parents=True, exist_ok=True)
    for p in written:
        shutil.copy2(p, SHARED / p.name)
    print(f"mirrored {len(written)} poster(s) → {SHARED}")
